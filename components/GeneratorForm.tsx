...
import { GenerationForm, Step, User, UserProfile } from '../types';
...
interface GeneratorProps {
  ...
  onModeChange: (mode: 'cover' | 'ebook' | 'mockup' | 'ad' | 'video' | 'history') => void;
...
export const GeneratorForm: React.FC<GeneratorProps> = ({ sharedForm, onSharedFormUpdate, onModeChange, profile, user }) => {
  ...
  const [resultData, setResultData] = useState<{ download_url: string } | null>(null);

  const updateForm = (u: Partial<GenerationForm>) => onSharedFormUpdate({ ...sharedForm, ...u });
  const next = (d?: { download_url: string }) => {
    if (d) setResultData(d);
    setCurrentStep(s => (s as number) + 1);
  };
  const prev = () => setCurrentStep(s => (s as number) - 1);
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      {currentStep === Step.DASHBOARD && resultData && <DashboardStep resultData={resultData} onNext={() => next()} />}
